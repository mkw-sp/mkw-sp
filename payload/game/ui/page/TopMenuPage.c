...
void TopMenuPage_refreshFileAdminButton(TopMenuPage *this) {
    ...
    LayoutUIControl_setMiiPicture(&this->fileAdminButton->inherit, "mii", this->inherit.miiGroup,
            index, 2);
}
...
void my_TopMenuPage_onButtonSelect(TopMenuPage *this, PushButton *button) {
    if (button->index == -100) {
        ...
        CtrlMenuInstructionText_setMessage(this->inherit.instructionText, messageIds[button->index],
                NULL);
    }
}
