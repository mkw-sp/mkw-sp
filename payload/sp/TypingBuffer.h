...
static inline void TypingBuffer_Init(TypingBuffer *buf) {
    memset(buf->buf, 0, sizeof(buf->buf));
    buf->len = 0;
}

static inline bool TypingBuffer_IsExhausted(const TypingBuffer *buf) {
    return buf->len >= sizeof(buf->buf) - 1;
}

static inline void TypingBuffer_Append(TypingBuffer *buf, char c) {
    if (!TypingBuffer_IsExhausted(buf))
        buf->buf[buf->len++] = c;
}
static inline void TypingBuffer_Backspace(TypingBuffer *buf) {
