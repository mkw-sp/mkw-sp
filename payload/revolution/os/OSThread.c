...
BOOL OSCreateThread_RandomizeThreadStackPointer(OSThread *thread, void *(*func)(void *), void *param, void *stack, u32 stackSize, s32 priority, u16 attr)
{
    ...
    u32 bits_entropy = __builtin_ctz(old_stack_size >> THREAD_STACK_SIZE_RIGHT_SHIFT_AMOUNT);
    assert(bits_entropy != 0);

    u32* new_stack_base = RandomizeStackPointer(old_stack_base, bits_entropy);
