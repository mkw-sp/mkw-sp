use anyhow::bail;
use netprotocol::{async_stream::AsyncStream, matchmaking::{self, CTSMessageOpt, CTSMessage}};
use libhydrogen::{kx, secretbox};
use tokio::net::ToSocketAddrs;

use crate::{Fallible, Result, cts_message, stc_message, STCMessage, STCMessageOpt, Server, ClientId};
...
impl ClientForwarder {
    pub async fn new(bind: impl ToSocketAddrs, server: Server, db_pool: sqlx::PgPool) -> Result<Self> {
        Ok(Self {
    ...
    async fn handle_connection(mut stream: AsyncStream<CTSMessageOpt, STCMessageOpt>, db_pool: sqlx::PgPool, server: Server) -> Fallible {
    ...
    async fn handle_login_flow(stream: &mut AsyncStream<CTSMessageOpt, STCMessageOpt>, db_pool: &sqlx::PgPool) -> Result<ClientId> {
        ...
        if initial_message.client_id.is_none() || initial_message.client_id.clone().unwrap().device == 0 {
            tracing::info!("Guest login");
            stream.write(STCMessageOpt { message: Some(STCMessage::GuestResponse(stc_message::LoginGuest {}))}).await?;
        ...
        stream.write(STCMessageOpt {message: Some(STCMessage::Challenge (stc_message::LoginChallenge {
        ...
        let rr_record = sqlx::query!(
            ...
            device_id, licence_id
        ).fetch_one(&mut db_connection).await?;


        stream.write(STCMessageOpt {message: Some(STCMessage::Response (stc_message::LoginResponse {
...
async fn fetch_friend_data(db_connection: &mut sqlx::PgConnection, device_id: i32, licence_id: i16) -> Result<Vec<stc_message::login_response::Friend>> {
    let fid_records = sqlx::query!(
        ...
        device_id, licence_id
    ).fetch_all(&mut *db_connection).await?;
